...
! real pole. We require that the residues of real poles are positive
      if (real(filter_S_PR(1)%residues(pole)).lt.0d0) then
...
! complex pole pair 
      ...
      ap=  real(filter_S_PR(1)%poles(pole))
      app=aimag(filter_S_PR(1)%poles(pole))
      cp=  real(filter_S_PR(1)%residues(pole))
      cpp=aimag(filter_S_PR(1)%residues(pole))
