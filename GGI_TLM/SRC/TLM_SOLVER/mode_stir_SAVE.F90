...
! count the number of faces contributing to this mode stirred boundary from this process
	  ...
	  if (rank.eq.np-1) then ! include zmax face 
	    n_faces=n_faces+nx*ny
          ...
          n_faces=n_faces+2*(nx*(nz2-nz1+1))+2*(ny*(nz2-nz1+1)) ! ymin, ymax, xmin, xmax faces
...
! mode stirred boundary to work correctly.
	  ...
	  n_faces=n_faces+problem_surfaces(surface_number)%number_of_faces
      ...
      mode_stir_surface_list(mode_stir_surface)%number_of_faces=n_faces
...
! The rank 0 process must be capable of storing all the mode stir voltage pulses for the surface
        ...
        CALL MPI_ALLREDUCE( n_faces,mode_stir_surface_list(mode_stir_surface)%total_number_of_mode_stir_faces	&
        ...
        mode_stir_surface_list(mode_stir_surface)%total_number_of_mode_stir_faces=n_faces
...
! allocate memory for the face list and the port voltage list
      ...
      ALLOCATE( mode_stir_surface_list(mode_stir_surface)%face_list(1:n_faces) )
      ALLOCATE( mode_stir_surface_list(mode_stir_surface)%port1(1:n_faces) )
      ALLOCATE( mode_stir_surface_list(mode_stir_surface)%port2(1:n_faces) )
...
! count the number of faces contributing to this mode stirred boundary from this process
	  ...
	  if (rank.eq.0) then ! include zmin face 
	    ...
	    do cx=1,nx
	      do cy=1,ny
	        n_faces=n_faces+1
		mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%cell%i=cx
		mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%cell%j=cy
		mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%cell%k=cz
		mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%point=face
		mode_stir_surface_list(mode_stir_surface)%port1(n_faces)=Vx_zmin
		mode_stir_surface_list(mode_stir_surface)%port2(n_faces)=Vy_zmin
	      end do
	    end do
	  ...
	  if (rank.eq.np-1) then ! include zmax face 
		...
		mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%cell%k=cz
		mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%point=face
		mode_stir_surface_list(mode_stir_surface)%port1(n_faces)=Vx_zmax
		mode_stir_surface_list(mode_stir_surface)%port2(n_faces)=Vy_zmax
	      end do
	    end do
...
! xmin face	  
	  ...
	  do cz=nz1,nz2
	    do cy=1,ny
	      ...
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%cell%k=cz
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%point=face
	      mode_stir_surface_list(mode_stir_surface)%port1(n_faces)=Vy_xmin
	      mode_stir_surface_list(mode_stir_surface)%port2(n_faces)=Vz_xmin
	    end do
	  end do
...
! xmax face	  
	  ...
	  do cz=nz1,nz2
	    do cy=1,ny
	      ...
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%cell%k=cz
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%point=face
	      mode_stir_surface_list(mode_stir_surface)%port1(n_faces)=Vy_xmax
	      mode_stir_surface_list(mode_stir_surface)%port2(n_faces)=Vz_xmax
	    end do
	  end do
...
! ymin face	  
	  ...
	  do cz=nz1,nz2
	    do cx=1,nx
	      ...
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%cell%k=cz
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%point=face
	      mode_stir_surface_list(mode_stir_surface)%port1(n_faces)=Vx_ymin
	      mode_stir_surface_list(mode_stir_surface)%port2(n_faces)=Vz_ymin
	    end do
	  end do
...
! ymax face	  
	  ...
	  do cz=nz1,nz2
	    do cx=1,nx
	      ...
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%cell%k=cz
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%point=face
	      mode_stir_surface_list(mode_stir_surface)%port1(n_faces)=Vx_ymax
	      mode_stir_surface_list(mode_stir_surface)%port2(n_faces)=Vz_ymax
	    end do
	  end do
...
! normal geometric surface
	  ...
	  do face_loop=1,problem_surfaces(surface_number)%number_of_faces
	    ...
	    if (orientation.eq.1) then
	      ...
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)=face1
	      
	    else
 ! get the opposite face	    
	      CALL get_other_side_of_face(face1,face2)
	      mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)=face2
	      
	    end if
	    
	    face=mode_stir_surface_list(mode_stir_surface)%face_list(n_faces)%point
...
! check
      if (n_faces.NE.mode_stir_surface_list(mode_stir_surface)%number_of_faces) then
        write(*,*)'Error in initialise_mode_stir_surfaces'
	write(*,*)'face counting discrepancy'
	write(*,*)n_faces,mode_stir_surface_list(mode_stir_surface)%number_of_faces
	STOP
      end if
