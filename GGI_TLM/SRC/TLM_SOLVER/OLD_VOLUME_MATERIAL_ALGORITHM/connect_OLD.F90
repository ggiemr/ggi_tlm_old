...
!  CALL write_line('CALLED: connect',0,output_to_screen_flag)
...
! faces normal to x i.e. the y z plane
  do cz=1,nz
	...
	if (face_update_code(face_number).eq.0) then   ! free space update, no output
...
!         face_update_code points to arrays which tell us about material number and output_number
	  ...
	  if (face_update_code_to_material_data(face_update_code(face_number),1).LT.0) then
...
! get the incident voltage pulses required for output field calculation	  
	  ...
	  if (material_type.EQ.0) then ! no material, simple connect
	    ...
	    if (material_type.EQ.surface_material_type_PEC) then
	      ...
	      V(Vy_xmin,cx,cy,cz)    =  -V(Vy_xmin,cx,cy,cz)
              V(Vy_xmax,cx-1,cy,cz)  =  -V(Vy_xmax,cx-1,cy,cz)
	
	      V(Vz_xmin,cx,cy,cz  )  =  -V(Vz_xmin,cx,cy,cz  )
              V(Vz_xmax,cx-1,cy,cz)  =  -V(Vz_xmax,cx-1,cy,cz)
	  
!	    else if (material_type.EQ.surface_material_type_PMC) then
!	  
!! scattered voltage =incident voltage so no action required	  
...
! get the incident voltage pulses required for output field calculation	  
	  ...
	  if (material_type.EQ.0) then ! no material, simple connect
	    ...
	    if (material_type.EQ.surface_material_type_PEC) then
	      ...
	      V(Vx_ymin,cx,cy,cz)  =  -V(Vx_ymin,cx,cy,cz)
              V(Vx_ymax,cx,cy-1,cz)=  -V(Vx_ymax,cx,cy-1,cz)
	
	      V(Vz_ymin,cx,cy,cz)  =  -V(Vz_ymin,cx,cy,cz)
              V(Vz_ymax,cx,cy-1,cz)=  -V(Vz_ymax,cx,cy-1,cz)
...
! get the incident voltage pulses required for output field calculation	  
	  ...
	  if (material_type.EQ.0) then ! no material, simple connect
	    ...
	    if (material_type.EQ.surface_material_type_PEC) then
	      ...
	      V(Vx_zmin,cx,cy,cz)  = -V(Vx_zmin,cx,cy,cz)
	      V(Vx_zmax,cx,cy,cz-1)= -V(Vx_zmax,cx,cy,cz-1)
       
	      V(Vy_zmin,cx,cy,cz)  = -V(Vy_zmin,cx,cy,cz)
	      V(Vy_zmax,cx,cy,cz-1)= -V(Vy_zmax,cx,cy,cz-1)  
...
!  CALL write_line('FINISHED: connect',0,output_to_screen_flag)
